...
import { SiDebian, SiGentoo, SiLinuxmint, SiUbuntu } from "react-icons/si";
import { getFrostCategory } from "@/lib/data/frost";

const category = getFrostCategory("linux");
const icons = { ubuntu: <SiUbuntu size={36} />, debian: <SiDebian size={36} />, "linux-mint": <SiLinuxmint size={36} />, gentoo: <SiGentoo size={36} /> };
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr 1fr", md: "repeat(4,1fr)" }, gap: 2.5 }}>
          {category.items.map((item, index) => <motion.div key={item.slug} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.08 }}>
            <Card component={Link} href={`/frost/linux/${item.slug}`} sx={{ height: "100%", textDecoration: "none", bgcolor: isDark ? "rgba(16,37,74,0.45)" : "background.paper", border: `1px solid ${isDark ? "rgba(125,211,252,0.1)" : "rgba(0,0,0,0.08)"}`, borderRadius: 3, transition: "all 0.22s", "&:hover": { borderColor: item.color, transform: "translateY(-3px)" } }}>
