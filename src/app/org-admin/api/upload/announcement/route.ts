...
export async function POST(request: NextRequest) {
  ...
  if (!file || !announcementId) {
    return NextResponse.json({ error: 'File and announcementId are required' }, { status: 400 });
  }

  const fileExt = file.name.split('.').pop();
  const filePath = `announcement-${announcementId}-${Date.now()}.${fileExt}`;
  const bytes = await file.arrayBuffer();

  // Store in 'announcements' bucket (images + PDFs)
